...
export function getData(topMessage: EmbedBuilder) {
	let currentCommand;
	let commandInfo = [];
	let moduleNum = 0;
	...
	commandInfo.forEach(command => {
		if (!embeds.find(embeds => embeds.data.title.toLowerCase() == command.module)) {
			...
			if (command.buttons) {
				for (let i = 0; i < (Object.keys(command.buttons).length); i++) {
					embed.addFields([
						{
							name: Object.keys(command.buttons)[i].charAt(0).toUpperCase() + Object.keys(command.buttons)[i].slice(1),
							value: "```" + command.buttons[Object.keys(command.buttons)[i]] + "```",
							inline: true,
						}
					]);
				}
				if (command.buttons["send"] && command.buttons["cancel"]) {
					embed.addFields([
						{
							name: "Timeout",
							value: "```" + (command.uses - command.buttons.send) + "```",
							inline: true,
						},
						{
							name: "Intuition",
							value: "```" + ((command.buttons.send / client.guilds.cache.size) * 100).toFixed(4) + "%```",
							inline: true,
						}
					]);
				}
			...
			let index = embeds.findIndex(embed => {
				return (embed.data.title).toLowerCase() == command.module;
			});

			embeds[Object.keys(embeds)[index]].addFields([
				{
					name: "\u200b",
					value: "\u200b",
					inline: false,
			...
			if (command.buttons) {
				...
				if (command.buttons["send"] && command.buttons["cancel"]) {
					embeds[Object.keys(embeds)[index]].addFields([
						{
							name: "Timeout",
							value: "```" + (command.uses - command.buttons.send - command.buttons.cancel) + "```",
							inline: true,
						},
						{
							name: "Intuition",
