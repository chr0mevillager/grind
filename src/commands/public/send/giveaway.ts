...
const deleteButtonRowDisabled = new ActionRowBuilder()
	.addComponents(
		new ButtonBuilder()
...
const deleteButtonRow = (messageID: string) => new ActionRowBuilder()
	.addComponents(
		new ButtonBuilder()
...
export async function interaction(interaction) {
	try {
		...
		const giveawayButtonRow = (uuid: string) => new ActionRowBuilder()
			.addComponents(
				new ButtonBuilder()
		...
		if (interaction.user.id == process.env.OWNER_ID) {
			giveaway = (results: string) => new EmbedBuilder()
				.setColor(colors.clearColor)
		...
		//Modal
		const inputModal = new ActionRowBuilder<TextInputBuilder>()
			.addComponents(
		...
		const modal = (uuid) => new ModalBuilder()
				...
				if (input != "") {
					...
					for (let i = 0; i < formattedWinners.length; i++) {
						dm.addFields([
							{
								name: "Winner #" + (i + 1),
								value: "<@" + winners[i] + ">" + (giveawayData[uuid].data[winners[i]] + "\u200b"),
								inline: true,
							}
						])
					}
