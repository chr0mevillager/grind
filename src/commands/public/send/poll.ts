...
export async function interaction(interaction) {
	try {
		...
		const deniedResponseMessage = new EmbedBuilder()
		...
		let dropdown = new SelectMenuBuilder()
		...
		let dropdownRow = (uuid: string) => new ActionRowBuilder()
		...
		if (interaction.user.id == process.env.OWNER_ID) {
			poll = (votingStatus: string) => new EmbedBuilder()
				.setColor(colors.clearColor)
				.setTitle(question)
		...
		async function updateMessage() {
			...
			updatedPoll.addFields([
				{
					name: pollButton1,
					value: "```" + pollResponses1 + votes1 + "```",
					inline: true,
				},
				{
					name: pollButton2,
					value: "```" + pollResponses2 + votes2 + "```",
					inline: true,
				},
			]);

			if (button3exists) updatedPoll.addFields([
				{
					name: pollButton3,
					value: "```" + pollResponses3 + votes3 + "```",
					inline: true,
				},
			]);
			if (button4exists) updatedPoll.addFields([
				{
					name: pollButton4,
					value: "```" + pollResponses4 + votes4 + "```",
					inline: true,
				}
			]);
